...
import json
import os
# Matplotlib scans the Windows font directory during import. Some embedded
# IDE Python environments omit WINDIR, so provide the standard value.
...
# Do not force a particular GPU here.  PyTorch/Accelerate will use the GPU
# selected by the local environment (normally CUDA device 0 on a single-GPU PC).
...
def main(args):
    ...
    seed = int(cfg.sampler.get('seed', 42))
    torch.manual_seed(seed)
    np.random.seed(seed)
    model_cfg = cfg.model
    # Keep the sampling seed in the model config so the diffusion sampler
    # does not silently override the experiment-level seed.
    model_cfg.seed = int(cfg.sampler.get('seed', 42))
    ...
    data_root = data_cfg.get('root', './data/archive/')
    ...
    if data_cfg['name'] == 'edge':
        ...
        dataset = loaders.RadioUNet_c(phase="test", dir_dataset=data_root)

    elif data_cfg['name'] == 'IRT4':
        dataset = loaders.RadioUNet_c_sprseIRT4(phase="test",dir_dataset=data_root, simulation="IRT4",carsSimul="no",carsInput="no")
    elif data_cfg['name'] == 'IRT4K':
        dataset = loaders.RadioUNet_c_sprseIRT4_K2(phase="test",dir_dataset=data_root, simulation="IRT4",carsSimul="no",carsInput="K2")
    elif data_cfg['name'] == 'DPMK':
        dataset = loaders.RadioUNet_c_K2(phase="test",dir_dataset=data_root, simulation="DPM",carsSimul="no",carsInput="K2")
    elif data_cfg['name'] == 'DPMCAR': #参数默认进入car_gain_image
        dataset = loaders.RadioUNet_c_WithCar_NOK_or_K(phase="test",dir_dataset=data_root, simulation="DPM", have_K2="no")
    elif data_cfg['name'] == 'DPMCARK': #参数默认进入car_gain_image
        dataset = loaders.RadioUNet_c_WithCar_NOK_or_K(phase="test",dir_dataset=data_root, simulation="DPM", have_K2="yes")
    elif data_cfg['name'] == 'MASK':
        dataset = loaders.RadioUNet_s(phase="test",dir_dataset=data_root,mask=True)
    elif data_cfg['name'] == 'MASK_R':
        dataset = loaders.RadioUNet_s(phase="test",dir_dataset=data_root)
    elif data_cfg['name'] == 'RANDOM':
        dataset = loaders.RadioUNet_s_random(phase="test",dir_dataset=data_root, mask=True)
    elif data_cfg['name'] == 'VERTEX':
        dataset = loaders.RadioUNet_s_vertex(phase="test",dir_dataset=data_root, mask=True)
    elif data_cfg['name'] == 'VERTEX_R':
        dataset = loaders.RadioUNet_s_vertex(phase="test",dir_dataset=data_root)
    else:
        raise NotImplementedError
...
class Sampler(object):
    def __init__(
        ...
        if self.accelerator.is_main_process:
            ...
            (self.results_folder / "config_resolved.yaml").write_text(cfg.dump(), encoding="utf-8")
            (self.results_folder / "run_info.json").write_text(json.dumps({
                "sample_num": int(sample_num),
                "batch_size": int(batch_size),
                "checkpoint": str(cfg.sampler.ckpt_path),
                "sampling_timesteps": int(cfg.model.sampling_timesteps),
                "seed": int(cfg.sampler.get('seed', 42)),
    ...
    def sample(self):
                    ...
                    for idx, batch in tqdm(enumerate(tmp_dl),  disable=not self.accelerator.is_main_process):
                        ...
                        if stage == 'INFERENCE':
                            ...
                            if idx == inference_stop_idx:
                                ...
                                # seaborn is only needed for an optional,
                                # commented-out KDE plot; keep inference lean.
        ...
        if accelerator.is_main_process and epoch_samples:
            summary = {k: float(v / epoch_samples) for k, v in metrics.items()}
            summary["brightest_point_distance_px"] = float(avg_dist)
            summary["num_samples"] = int(epoch_samples)
            (self.results_folder / "metrics.json").write_text(
